...
#dfgan的prepare_data只有一个data输入，dmgan-cl有两个
def prepare_data(data, text_encoder):
    ...
    #     torch.sort(caption_lens_2, 0, True)

    imgs_2 = imgs.clone()

    captions, sorted_cap_lens, sorted_cap_idxs = sort_sents(captions, caption_lens)
    captions_2,sorted_cap_lens_2, sorted_cap_idxs_2 = sort_sents(captions_2,caption_lens_2)
    ...
    sent_emb_2 = rm_sort(sent_emb_2, sorted_cap_idxs_2)
    words_embs_2 = rm_sort(words_embs_2, sorted_cap_idxs_2)

    imgs = Variable(imgs).cuda()
    imgs_2 = Variable(imgs_2).cuda()
    return imgs,imgs_2, sent_emb,sent_emb_2, words_embs,words_embs_2, keys,sorted_cap_idxs,sorted_cap_idxs_2


def prepare_damsm_data(data):
    ...
    # Variable是用来求导的向量，在后期版本和tensor一样
...
def encode_tokens(text_encoder, caption, cap_lens):
    ...
    with torch.no_grad():
        ...
        words_embs, sent_emb = text_encoder(caption) #bert
...
def get_imgs(img_path, imsize, bbox=None,
    ...
    if cfg.GAN.B_DCGAN:
        ...
        for i in range(cfg.TREE.BRANCH_NUM):
            # print(imsize[i])
            #代码改动：IndexError: list index out of range
...
#对应from datasets import TextDataset
